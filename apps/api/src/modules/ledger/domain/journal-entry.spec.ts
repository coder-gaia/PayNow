...
describe('assertBalanced', () => {
  it('aceita o lancamento de emissao de fatura do plano de contas', () => {
    expect(() =>
      assertBalanced({
        lines: [
  ...
  it('recusa lancamento que nao fecha, dizendo quanto sobrou', () => {
    expect(() =>
      assertBalanced({
        lines: [
          line(ACCOUNT.CUSTOMER_RECEIVABLE, brl('100.00')),
  ...
  it('recusa lancamento com uma perna so', () => {
    expect(() =>
      assertBalanced({ lines: [line(ACCOUNT.CUSTOMER_RECEIVABLE, brl('100.00'))] }),
    ).toThrow(EmptyEntryError);
  });

  it('recusa lancamento vazio', () => {
    expect(() => assertBalanced({ lines: [] })).toThrow(EmptyEntryError);
  });

  it('balanceia por moeda, e nao no total', () => {
    // As duas moedas se anulam se somadas ingenuamente como numeros, mas cada
    // uma sozinha esta desbalanceada. Somar tudo junto deixaria isso passar.
    expect(() =>
      assertBalanced({
        lines: [
          line(ACCOUNT.CUSTOMER_RECEIVABLE, brl('100.00')),
  ...
  it('aceita lancamento que fecha em cada moeda separadamente', () => {
    expect(() =>
      assertBalanced({
        lines: [
          line(ACCOUNT.CUSTOMER_RECEIVABLE, brl('100.00')),
...
describe('totais do lancamento', () => {
  ...
  it('soma os creditos em valor absoluto', () => {
    expect(creditTotals(pagamento).get('BRL')?.toDecimalString()).toBe('103.00');
  });

  it('debitos e creditos batem, que e o que significa estar balanceado', () => {
